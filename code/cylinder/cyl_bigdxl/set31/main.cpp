...
double WSV = 3.*4.5951*SIGSV/XI;
double WLV = 3.*4.5951*SIGLV/XI;
double WSL = 3.*4.5951*SIGSL/XI;
//double EPS = sqrt(6.*SIG*XI/2.94);
double EPSSV = sqrt(6.*SIGSV*XI/4.5951);
double EPSLV = sqrt(6.*SIGLV*XI/4.5951);
double EPSSL = sqrt(6.*SIGSL*XI/4.5951);
...
double dt = pow(dxl,4.)/(12.*EPSMAX*EPSMAX*EMM)*tstable/3.;
double time = 0.;
int itime = 0;
...
void initialize(double p[nphase][nxtot][nytot][nztot])
{
	...
	clean_all(p);
        ...
        fill_cylinder(1,p,nx/2,ny/2,8+36/2/2,'z',48/2/2,36/2);
        fill_environ(0,p);
        relax_p(p,500);
}
...
void solvPF_CH(double pnew[nphase][nxtot][nytot][nztot], double pold[nphase][nxtot][nytot][nztot])
{
	...
	dt = pow(dxl,4.)/(12.*EPSMAX*EPSMAX*EMM)*tstable/3.;
...
double calc_frac_vol(double p[nxtot][nytot][nztot])
{
	/////////////
	double sum=0.;
        double pp;
                ...
                for (mp=0;mp<nphase;mp++) {
                    ...
                    for (np=0;np<nphase;np++) {
                       if (lp!=mp && mp!=np && np!=lp) {
                           dFdphi[lp] += 16.*WSV*dtg(p[lp][iw][jw][kw],p[mp][iw][jw][kw],p[np][iw][jw][kw]);
                       }
                    }
...
void calc_dG_rlx(double dG[nphase][nphase], double p[nphase][nxtot][nytot][nztot],int i,int j,int k)
{
	...
	for (lp=0;lp<nphase;lp++) {
                ...
                for (mp=0;mp<nphase;mp++) {
                    if (lp!=mp) {
                        dFdphi[lp] += EPSrlx*EPSrlx*lapl[mp]/2.;
                        if(mp>lp)dFdphi[lp] += Wrlx*dg_rlx(p[lp][iw][jw][kw],p[mp][iw][jw][kw]);
                    }
                }
	}
        for (lp=0;lp<nphase;lp++) {
...
void frac_out(double p[nphase][nxtot][nytot][nztot])
{
    ...
    //fprintf(out,"%d\t%lf\t%lf\t%lf\n",itime,calc_frac_vol(p[0]),calc_frac_vol(p[1]),calc_frac_vol(p[2]));
