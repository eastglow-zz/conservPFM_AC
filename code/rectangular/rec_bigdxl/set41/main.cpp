...
void initialize(double p[nphase][nxtot][nytot][nztot])
{
	...
	clean_all(p); 
        ...
        draw_rectangular(p[1],nx/2,ny/2,10,92,116,20);
        fill_rectangular(2,p,nx/2,ny/2,20+15,92,116,30);
        //draw_cylinder(p[1],nx/2,ny/2,10,'z',48,20);
        fill_environ(0,p);
        relax_p(p, 50);
        //clean_one(p[0]);
        //fill_rectangular(2,p,nx/2,ny/2,20+25-2,92,116,50);
        //fill_cylinder(2,p,nx/2,ny/2,20+25-2,'z',48,50);
        //fill_environ(0,p);
}
                ...
                for (mp=0;mp<nphase;mp++) {
                    ...
                    for (np=0;np<nphase;np++) {
                       if (lp!=mp && mp!=np && np!=lp) {
                           //dFdphi[lp] += 5.*WSV*dtg(p[lp][iw][jw][kw],p[mp][iw][jw][kw],p[np][iw][jw][kw]);
                       }
                    }
