...
void initialize(double p[nphase][nxtot][nytot][nztot])
{
	...
	///////////
	clean_all(p);
        draw_rectangular(p[2],nx/2,ny/2,4/2,nx,ny,8/2);
        draw_rectangular(p[2],nx/2,ny/2,(8+15)/2,46/2,58/2,30/2);
        fill_environ(0,p);
        relax_p(p,50);
        clean_one(p[0]);
        fill_rectangular(1,p,nx/2,ny/2,(8+30+13-4)/2,46/2,58/2,30/2);
        fill_environ(0,p);
}
