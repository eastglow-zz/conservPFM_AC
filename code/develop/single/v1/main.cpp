...
#define ii(i) i+1
#define ic(i,j,k) (i)*nytot*nztot+(j)*nztot+(k)
#define OLD(x) x%2
#define NEW(x) !(x%2)
...
int main()
{
	...
	void fileout_dg(void);
        void calc_s(double s[nphase][ntot],double p[nphase][ntot]);
	void initialize(double p[nphase][ntot]);
	void update_p(double pold[nphase][ntot], double pnew[nphase][ntot]);
	void BC_periodic3D(double p[ntot]);
	void BC_adiabatic3D(double p[ntot]);
	void BC_userdefine3D(double p[ntot]);
	void solvPF_CH(double pnew[nphase][ntot], double pold[nphase][ntot]);
	void solvPF_CA(double pnew[nphase][ntot], double pold[nphase][ntot]);
	void drivPF_CA(double pnew[nphase][ntot], double pold[nphase][ntot]);
        void calc_lamb(double pnew[nphase][ntot], double pold[nphase][ntot]);
	void fileout_p(double p[nphase][ntot]);
	double calc_frac_p(double p[ntot]);
        void frac_out(double p[nphase][ntot]);
        void clean_RR_HH(void);
	/////////////////////
        double p[2][nphase][ntot]; //phase field old/new switchable
	//double pnew[nphase][ntot];
	double pinter[nphase][ntot];
	/////////////////////
	//fileout_g();
        //fileout_dg();
...
{
        ///////////
        void clean_all(double p[nphase][ntot]);
        void clean_one(double p[ntot]);
        void relax_p(double p[nphase][ntot], int nrelax);
        void copy_p(double presult[nphase][ntot], double psource[nphase][ntot]);
        void draw_cube(double p[ntot],int ix, int iy, int iz, int size);
        void draw_sphere(double p[ntot],int ix, int iy, int iz, int diameter);
        void draw_cylinder(double p[ntot],int ix, int iy, int iz, char axis, int rad, int height);
        void draw_rectangular(double p[ntot],int ix, int iy, int iz, int sizex, int sizey,int sizez);
        void fill_rectangular(int phaseindex,double p[nphase][ntot],int ix, int iy, int iz, int sizex, int sizey, int sizez);
        void fill_cylinder(int phaseindex,double p[nphase][ntot],int ix, int iy, int iz, char axis, int rad, int height);
        void fill_environ(int phaseindex,double p[nphase][ntot]);
	///////////
	clean_all(p); 
        //draw_cylinder(p[2],nx/2,ny/2,nz/2+6,'z',16,10);
        //fill_cylinder(1,p,nx/2,ny/2,nz/2-6,'z',16,10);
        draw_rectangular(p[1],nx/2,ny/2,5 ,nx,ny,10);
        fill_environ(0,p);
        relax_p(p, 50);
        clean_one(p[0]);
...
{
	//////////////////
	void calc_dG(double dG[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
        void calc_EMM(double EM[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
	//////////////////
	int i,j,k;
...
void relaxPF_CH(double pnew[nphase][ntot], double pold[nphase][ntot])
{
	//////////////////
	void calc_dG(double dG[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
	//////////////////
...
void solvPF_CA(double pnew[nphase][ntot], double pold[nphase][ntot])
{
	...
	void calc_dG(double dG[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
        void calc_EMM(double EM[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
        void trim_p(double p[nphase][ntot],int iw,int jw,int kw);
	//////////////////
	int i,j,k;
...
{
	//////////////////
	void calc_dDRIV(double dG[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
        void calc_EMM(double EM[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
        void trim_p(double p[nphase][ntot],int iw,int jw,int kw);
	//////////////////
	int i,j,k;
...
void relaxPF_CA(double pnew[nphase][ntot], double pold[nphase][ntot])
{
	//////////////////
	void calc_dG_rlx(double dG[nphase][nphase], double p[nphase][ntot],int i,int j,int k);
        void trim_p_rlx(double p[nphase][ntot],int iw,int jw,int kw);
	//////////////////
	int i,j,k;
...
double df(int np, double p1[nphase][ntot],int iw,int jw,int kw)
{
    /////
...
{
        /////////////
        int makephaselist(int plist[nphase],double p[nphase][ntot],int iw, int jw, int kw);
        /////////////
...
void calc_s(double s[nphase][ntot],double p[nphase][ntot])
{
    ...
    for (k=1;k<=nz;k++) {
        for (np=0;np<nphase;np++) {
            if (p[np][ic(ii(i),ii(j),ii(k))]>0.) {
            //if (np!=2) {
                s[np][ic(ii(i),ii(j),ii(k))]=1.;
            }else{
                s[np][ic(ii(i),ii(j),ii(k))]=0.;
            }
        }
    }
    }
...
void draw_sphere(double p[ntot],int ix, int iy, int iz, int size)
{
    // make a cube at position (i,j,k)
    //////////////////////////////////
    int i,j,k;
...
{
    //////////////
    double df(int,double p[nphase][ntot],int,int,int);
    //////////////
    extern double lamb[nphase];
