        ...
        for (mp=0;mp<nphase;mp++) {
            ...
            for (np=0;np<nphase;np++) {
                if (lp!=mp && mp!=np && np!=lp) {
                    //dFdphi[lp] += 5.*WSV*dtg(p[lp][ic(iw,jw,kw)],p[mp][ic(iw,jw,kw)],p[np][ic(iw,jw,kw)]);
                }
            }
	...
	for (k=1;k<=nz;k++) {
		...
		for (lp=0;lp<nphase;lp++) {
		    ...
		    pnew[lp][ic(ii(i),ii(j),ii(k))]=pold[lp][ic(ii(i),ii(j),ii(k))]+delta[lp]*dt;
                    //if (pnew[lp][ic(ii(i),ii(j),ii(k))]<=0.) pnew[lp][ic(ii(i),ii(j),ii(k))]=0.;
                    //if (pnew[lp][ic(ii(i),ii(j),ii(k))]>=1.) pnew[lp][ic(ii(i),ii(j),ii(k))]=1.;
		}
