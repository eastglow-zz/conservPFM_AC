...
double dmax4(double a1, double a2, double a3, double a4); 
double dmax(double a[],int np); 
...
double EPSSL2 = sqrt(6.*SIGSL2*XI/2.94);
double EPSMAX = dmax4(EPSSV,EPSLV,EPSSL,EPSSL2);
double EMM=1.;
...
void initialize(double p[nphase][nxtot][nytot][nztot])
{
	...
	///////////
	clean_all(p);
        //draw_rectangular(p[3],nx/2,ny/2,2,nx,ny/4,4);
        fill_rectangular(2,p,nx/2,ny/2,2,nx,ny,4);
        fill_environ(0,p);
        relax_p(p,1000);
        clean_one(p[0]);
        fill_rectangular(1,p,nx/2,ny/2,4+6,16,16,18);
        fill_environ(0,p);
}
	...
	for (k=1;k<=nz;k++) {
		...
		for (mp=0;mp<nphase;mp++) {
			if (lp!=mp) {
				...
				dJx[lp][mp]=(EMMp(lp,mp)*gradGxp[lp][mp]-EMMp(lp,mp)*gradGxm[lp][mp])/dxl;
				dJy[lp][mp]=(EMMp(lp,mp)*gradGyp[lp][mp]-EMMp(lp,mp)*gradGym[lp][mp])/dyl;
				dJz[lp][mp]=(EMMp(lp,mp)*gradGzp[lp][mp]-EMMp(lp,mp)*gradGzm[lp][mp])/dzl;
...
void fileout_p(double p[nphase][nxtot][nytot][nztot])
{
	...
	for (j=1;j<=ny;j++) {
	for (i=1;i<=nx;i++) {
		fprintf(out,"%d\t%d\t%d\t%lf\t%lf\t%lf\t%lf\n",i,j,k,p[0][ii(i)][ii(j)][ii(k)],p[1][ii(i)][ii(j)][ii(k)],p[2][ii(i)][ii(j)][ii(k)],p[3][ii(i)][ii(j)][ii(k)]);
	}
	}
	}
	fclose(out);
...
void frac_out(double p[nphase][nxtot][nytot][nztot])
{
    ...
    fprintf(out,"%d\t%lf\t%lf\t%lf\t%lf\n",itime,calc_frac_p(p[0]),calc_frac_p(p[1]),calc_frac_p(p[2]),calc_frac_p(p[3]));
    fclose(out);
}
...
double SIG(int np1, int np2)
{
    ...
    if ((np1==0 && np2==3) || (np1==3 && np2==0)) {
        result = SIGSV;
    }
    if ((np1==1 && np2==2) || (np1==2 && np2==1)) {
        result = SIGSL;
    }
    if ((np1==1 && np2==3) || (np1==3 && np2==1)) {
        result = SIGSL2;
    }
    if (np1==np2) {
        result = 0.;
